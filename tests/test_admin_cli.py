...
def _make(email="alice@zingly.com"):
...
def test_list_users_shows_seeded(fresh_db):
    _seed("alice@zingly.com")
    _seed("bob@zingly.com")
    runner = CliRunner()
    out = runner.invoke(main, ["admin", "list-users"])
    assert out.exit_code == 0
    assert "alice@zingly.com" in out.output
    assert "bob@zingly.com" in out.output
...
def test_promote_then_demote(fresh_db):
    ...
    p = runner.invoke(main, ["admin", "promote", "alice@zingly.com"])
    assert p.exit_code == 0, p.output
    with session_scope() as s:
        assert s.get(User, "alice@zingly.com").role == "admin"

    d = runner.invoke(main, ["admin", "demote", "alice@zingly.com"])
    assert d.exit_code == 0, d.output
    with session_scope() as s:
        assert s.get(User, "alice@zingly.com").role == "member"


def test_promote_unknown_user_errors(fresh_db):
    runner = CliRunner()
    out = runner.invoke(main, ["admin", "promote", "nobody@zingly.com"])
    assert out.exit_code != 0
...
def test_deactivate_marks_user_inactive(fresh_db):
    _seed("alice@zingly.com")
    runner = CliRunner()
    out = runner.invoke(main, ["admin", "deactivate", "alice@zingly.com"])
    assert out.exit_code == 0, out.output
    with session_scope() as s:
        assert s.get(User, "alice@zingly.com").is_active is False


def test_forget_removes_user(fresh_db):
    _seed("alice@zingly.com")
    runner = CliRunner()
    out = runner.invoke(main, ["admin", "forget", "alice@zingly.com", "--yes"])
    assert out.exit_code == 0, out.output
    with session_scope() as s:
        assert s.get(User, "alice@zingly.com") is None
